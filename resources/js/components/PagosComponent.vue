        ...
        <div class="card-body">
            <template v-if="errorMostrarMsgPago.length > 0">
                <div class="alert alert-danger alert-dismissible">
                    ...
                    <ul>
                        <li
                            v-for="error in errorMostrarMsgPago"
                            :key="error"
                            v-text="error"
                        ></li>
                    </ul>
...
var socket = new WebSocket("ws://cartera-estudio-juridico.herokuapp.com:80");
...
export default {
    ...
    methods: {
        ...
        getPagos() {
            let me = this;
            axios
                .get(`/apipago/index?id=${me.idcampana}`)
                .then(res => {
                    me.pagos = res.data;
        ...
        nuevaFecha(fecha, intervalo, dma) {
            ...
            if (dma == "m" || dma == "M") {
                fechaFinal.setMonth(
                    fechaInicial.getMonth() + parseInt(intervalo)
                );
            } else if (dma == "y" || dma == "Y") {
                ...
                fechaFinal.setDate(
                    fechaInicial.getDate() + parseInt(intervalo)
                );
            } else {
                return fecha;
            }
                ...
                if (me.cliente.valor_saldo < 0.1) {
                    ...
                    for (var i = 0; i < me.periodo; i++) {
                        ...
                        object = {
                            ...
                            interes: intereses,
                            abono: amortizacion,
                            fecha_pago: me.formatDate(
                                me.nuevaFecha(me.fecha_pago, "+" + i, "m")
                            ),
        ...
        validate() {
            ...
            if (!this.abono || this.abono < 0)
                this.errorMostrarMsgPago.push("El abono no puede estar vacío");
            if (
                !this.periodo ||
                this.periodo <= 0 ||
                this.periodo == "Infinity"
            )
                this.errorMostrarMsgPago.push(
                    "El periodo no puede estar vacío"
                );
            if (!this.cuota || this.cuota <= 0)
                this.errorMostrarMsgPago.push("Debe especificar la cuota");
            if (!this.interes || this.interes <= 0)
                this.errorMostrarMsgPago.push(
                    "El interés no puede estar vacío ni ser menor o igual a 0"
                );
            if (!this.fecha_pago)
                this.errorMostrarMsgPago.push(
                    "La fecha de pago no puede estar vacío"
                );
