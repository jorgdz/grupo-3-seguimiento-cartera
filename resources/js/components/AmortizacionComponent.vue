...
export default {
    ...
    computed: {
        calcularPeriodo() {
            let me = this;
            me.periodos = Math.round(
                (parseFloat(me.saldo_deuda) - parseFloat(me.abono)) /
                    parseFloat(me.monto)
            );
            return NP.round(me.periodos, 0);
        }
    },
    ...
    methods: {
        calcularAmortizacion() {
            ...
            let denominador = Math.pow(
                ...
                parseFloat(me.periodos)
            );

            let tmpCuotaFija =
                (parseFloat(interesDecimal) * parseFloat(monto_cobrar)) /
                (1 - parseFloat(denominador));

            let cuota_fija = NP.round(tmpCuotaFija, 3);
            ...
            for (var i = 1; i <= me.periodos; i++) {
                intereses = NP.round(
                    parseFloat(monto_cobrar) * parseFloat(me.interes / 100),
                    3
                );
